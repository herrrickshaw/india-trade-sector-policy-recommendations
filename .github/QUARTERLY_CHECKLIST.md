...
- [ ] **Quarterly Trade Watch PDF** — update the figures in `scripts/build_quarterly_report.py` (Highlights, §1–§7 — each carries its source) and re-run `/opt/homebrew/bin/python3 scripts/build_quarterly_report.py` (single WeasyPrint pass, ~1s; Homebrew python required — SIP blocks system Python's pango). Rotate the special-theme chapter each edition per NITI Trade Watch practice.
